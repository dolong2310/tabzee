function Tabzee(selector, options = {}) {
  this.container = document.querySelector(selector);
  if (!this.container) {
    console.error(`Tabzee: No container found for selector '${selector}'`);
    return;
  }

  this.tabs = Array.from(this.container.querySelectorAll("li a"));
  if (!this.tabs.length) {
    console.error(`Tabzee: No tabs found inside the container`);
    return;
  }
...
Tabzee.prototype.getPanels = function () {
  return this.tabs
    .map((tab) => {
      const panel = document.querySelector(tab.getAttribute("href"));
      if (!panel) {
        console.error(
          `Tabzee: No panel found for selector '${tab.getAttribute("href")}'`
        );
      }
      return panel;
    })
...
Tabzee.prototype._init = function () {
...
Tabzee.prototype._tryActivateTab = function (tab) {
...
Tabzee.prototype._activateTab = function (
  tab,
...
Tabzee.prototype.switch = function (input) {
  const tab =
    typeof input === "string"
  ...
  if (!tab) {
    console.error(`Tabzee: Invalid input '${input}'`);
    return;
  }
  this._tryActivateTab(tab);
};

Tabzee.prototype.destroy = function () {
